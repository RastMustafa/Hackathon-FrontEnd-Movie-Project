...
function SearchBar() {
  ...
  return (
    <div className="flex flex-row-reverse text-white items-center w-full h-[42px] rounded-lg focus-within:shadow-lg overflow-hidden">
      <Link href={`${inputValue}` === "" ? "/" : `/search?query=${inputValue}`}>
        <div
          onClick={() => handleSearchQuery()}
          className="grid   pr-6 place-items-center rounded-2xl -ml-8 h-full w-12 text-gray-300"
        >
          <FiSearch />
        </div>
      </Link>
